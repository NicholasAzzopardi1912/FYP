...
y_arousal = video_df["arousal_class"].values
y_valence = video_df["valence_class"].values
...
def build_classification_video_model(input):
    model = Sequential([
...
def group_kfold_split(X, y, groups, label_name = "Arousal"):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups = groups), 1):
        ...
        model = build_classification_video_model(X_train.shape[1])
    ...
    results_df.to_csv(f"{label_name.lower()}_classification_video_groupkfold_results.csv", index=False)
    print(f"\nSaved fold results to {label_name.lower()}_classification_video_groupkfold_results.csv")
