# Experiment 4: Testing the model when alpha is equal to a very small value close to 0 (observing whether results will be similar to that of the respective baseline regression model)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            if self.alpha == 0.0:
                ...
                # Squaring the representation loss to keep the representation term small when alpha is tiny
...
def train_student_regressor(X_audio, X_video, X_physio, y, groups, target_name = "arousal", alpha=0.0, student_epochs=50, student_batch_size=32, teacher_epochs=200, teacher_batch_size=64, teacher_patience=15, student_patience=8):
    ...
    out_csv = f"audio_student_regressor_EXP4_{target_name}_alpha_{alpha}.csv"
...
# Running the training for both arousal and valence targets across the tiny alpha value to observe if results are similar to the respective baseline regression model
