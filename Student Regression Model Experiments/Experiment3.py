# Experiment 3: Training the student on video data instead of audio data to analyse whether LUPI behaves differently on a weaker modality.
...
def build_video_student_regression_model(input_dimensions):
    # Student model input -> video features only
    video_input = layers.Input(shape=(input_dimensions,), name="video_student_input")

    # First hidden layer for video regression
    x = layers.Dense(128, activation="relu", name="dense_128")(video_input)
    # Dropout for regularization
    x = layers.Dropout(0.3, name="drop_1")(x)

    # Second hidden layer for video regression
    ...
    # Defining the video-only student regression model
    model = Model(inputs=video_input, outputs=outputs, name="video_student_regression")
...
class LUPIStudentRegressor(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # Video is being used by the student, while audio and physio are only for the teacher's representation loss
        audio_x = x["audio"]
        video_x = x["video"]
        physio_x = x["physio"]

        with tf.GradientTape() as tape:
            # Student prediction from video-only input
            y_hat = self.student_model(video_x, training=True)
            ...
            if self.alpha == 0.0:
                ...
                # Student representation from video-only input
    ...
    def test_step(self, data):
        # Evaluation step using video-only input
        x, y = data
        ...
        # Student prediction
        y_hat = self.student_model(video_x, training=False)
        # Regression Loss
        mse_loss = tf.reduce_mean(tf.keras.losses.mse(y, y_hat))
...
def train_student_regressor(X_audio, X_video, X_physio, y, groups, target_name = "arousal", alpha=0.0, student_epochs=50, student_batch_size=32, teacher_epochs=200, teacher_batch_size=64, teacher_patience=15, student_patience=8):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X_audio, y, groups=groups), start=1):
        ...
        # Splitting video data and targets into training and testing sets based on the current fold's indices
        ...
        if alpha == 0.0:
            # Training video-only student with standard supervised loss
            student = build_video_student_regression_model(X_video_train.shape[1])
            ...
            y_pred = student.predict(X_video_test, verbose=0).flatten()
            ...
            # Prediction using video-only student path at test time
            y_pred = lupi_model.predict(X_video_test, verbose=0).flatten()
    ...
    out_csv = f"video_student_regressor_EXP3_{target_name}_alpha_{alpha}.csv"
