# Physiological Regression Student Model with LUPI
...
RUN_DIR = "runs_physio_student_regression"
...
def build_physio_student_regression_model(input_dimensions):
    # Student model input -> physiological features only
    physio_input = layers.Input(shape=(input_dimensions,), name="physio_student_input")

    # First hidden layer for physiological regression
    x = layers.Dense(128, activation="relu", name="dense_128")(physio_input)
    # Dropout for regularization
    x = layers.Dropout(0.3, name="drop_1")(x)

    # Second hidden layer for physiological regression
    ...
    # Defining the physiological-only student regression model
    model = Model(inputs=physio_input, outputs=outputs, name="physio_student_regression")
...
class LUPIStudentRegressor(tf.keras.Model):
    ...
    def call(self, physio_x, training=False):
        # Forward pass uses the underlying student model
        return self.student_model(physio_x, training=training)
    
    def train_step(self, data):
        # Unpacking the data, where x contains all modalities and y is the target
        x, y = data

        # Physio is being used by the student, while audio and video are only for the teacher's representation loss
        audio_x = x["audio"]
        video_x = x["video"]
        physio_x = x["physio"]

        with tf.GradientTape() as tape:
            # Student prediction from physio-only input
            y_hat = self.student_model(physio_x, training=True)
            ...
            if self.alpha == 0.0:
                ...
                # Student representation from physio-only input
                S = self.student_rep_model(physio_x, training=True)
    ...
    def test_step(self, data):
        # Evaluation step using physio-only input
        x, y = data
        ...
        # Student prediction
        y_hat = self.student_model(physio_x, training=False)
        # Regression Loss
        mse_loss = tf.reduce_mean(tf.keras.losses.mse(y, y_hat))
...
def train_student_regressor(X_audio, X_video, X_physio, y, groups, target_name = "arousal", alpha=0.0, student_epochs=50, student_batch_size=32, teacher_epochs=200, teacher_batch_size=64, teacher_patience=15, student_patience=8):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X_physio, y, groups=groups), start=1):
        print(f"\nFold {fold}/{n_splits}")

        fold_dir = os.path.join(RUN_DIR, f"{target_name}", f"alpha_{alpha}", f"fold_{fold:02d}")
        os.makedirs(fold_dir, exist_ok=True)

        # Splitting physio data and targets into training and testing sets based on the current fold's indices
        ...
        if alpha == 0.0:
            # Training physio-only student with standard supervised loss
            student = build_physio_student_regression_model(X_physio_train.shape[1])
            ...
            y_pred = student.predict(X_physio_test, verbose=0).flatten()
            ...
            # Prediction using physio-only student path at test time
            y_pred = lupi_model.predict(X_physio_test, verbose=0).flatten()
    ...
    out_csv = f"physio_student_regressor_{target_name}_alpha_{alpha}.csv"
