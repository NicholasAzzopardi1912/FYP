...
RUN_DIR = "runs_physio_student_classification_combined"
...
def build_physio_student_classification_model(input_dimensions):
    # Input layer for physiological features
    physio_input = layers.Input(shape=(input_dimensions,), name="physio_student_input")

    # First hidden layer for physiological classification
    x = layers.Dense(128, activation="relu", name="dense_128")(physio_input)
    # Dropout for regularization
    x = layers.Dropout(0.3, name="dropout_1")(x)

    # Second hidden layer for physiological classification
    ...
    # Defining the physiological-only student classification model
    model = Model(inputs=physio_input, outputs=outputs, name="physio_student_classifier")
...
class LUPIStudentClassifierCombined(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # Physio is used by the student, while audio and video are only used by the teacher
        audio_x = x["audio"]
        video_x = x["video"]
        physio_x = x["physio"]

        with tf.GradientTape() as tape:
            # Student prediction using physiological input
            student_probability = self.student_model(physio_x, training=True)
            ...
            if self.alpha == 0.0:
                ...
                # Student representation from physiological input
    ...
    def test_step(self, data):
        # Evaluation step using physiological-only input (no privileged information)
        x, y = data
        ...
        student_probability = self.student_model(physio_x, training=False)
...
def train_student_classifier_combined(X_audio, X_video, X_physio, y, groups, target_name="arousal", alpha=0.0, student_epochs=50, student_batch_size=32, teacher_epochs=200, teacher_batch_size=64, teacher_patience=15, student_patience=8):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X_physio, y, groups=groups), start=1):
        ...
        # Splitting physiological and targets into train and test sets
        ...
        if alpha == 0.0:
            ...
            student = build_physio_student_classification_model(X_physio_train.shape[1])
            ...
            y_prob = student.predict(X_physio_test, verbose=0).flatten()
            ...
            # Probabilistic prediction using physiological-only path at test time
            y_prob = lupi_model.predict(X_physio_test, verbose=0).flatten()
    ...
    out_csv = f"physio_student_classifier_Combined_{target_name}_alpha_{alpha}.csv"
