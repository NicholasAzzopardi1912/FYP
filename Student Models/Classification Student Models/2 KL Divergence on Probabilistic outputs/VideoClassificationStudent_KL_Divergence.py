...
RUN_DIR = "runs_video_student_classification_kl"
...
class LUPIStudentClassifier(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # Video is used by the student while audio and physio are only used to compute the frozen teacher predictions
        audio_x = x["audio"]
        video_x = x["video"]
        physio_x = x["physio"]
...
def train_student_classifier(X_audio, X_video, X_physio, y, groups, target_name="arousal", alpha=0.0, student_epochs=50, student_batch_size=32, teacher_epochs=200, teacher_batch_size=64, teacher_patience=15, student_patience=8):
    ...
    out_csv = f"video_student_classifier_KL_Divergence_{target_name}_alpha_{alpha}.csv"
