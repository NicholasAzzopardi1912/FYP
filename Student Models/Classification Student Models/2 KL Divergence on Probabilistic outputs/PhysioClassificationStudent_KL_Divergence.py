...
RUN_DIR = "runs_physio_student_classification_kl"
...
def build_physio_student_classification_model(input_dimensions):
    ...
    # Defining the physio-only student classification model
...
class LUPIStudentClassifier(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # Physio is used by the student while audio and video are only used to compute the frozen teacher predictions
        audio_x = x["audio"]
        video_x = x["video"]
        physio_x = x["physio"]
    ...
    def test_step(self, data):
        # Evaluation step using physio-only input (no privileged information)
        x, y = data
...
def train_student_classifier(X_audio, X_video, X_physio, y, groups, target_name="arousal", alpha=0.0, student_epochs=50, student_batch_size=32, teacher_epochs=200, teacher_batch_size=64, teacher_patience=15, student_patience=8):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X_physio, y, groups=groups), start=1):
        ...
        # Splitting physio and targets into train and test sets
    ...
    out_csv = f"physio_student_classifier_KL_Divergence_{target_name}_alpha_{alpha}.csv"
