...
RUN_DIR = "runs_physio_student_classification_cosine"
...
class LUPIStudentClassifier(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # Physio is used by the student, while audio and video are only used by the teacher for representation learning
        audio_x = x["audio"]
        video_x = x["video"]
        physio_x = x["physio"]
...
def train_student_classifier(X_audio, X_video, X_physio, y, groups, target_name="arousal", alpha=0.0, student_epochs=50, student_batch_size=32, teacher_epochs=200, teacher_batch_size=64, teacher_patience=15, student_patience=8):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X_physio, y, groups=groups), start=1):
        ...
        if alpha == 0.0:
            ...
            # Probabilistic prediction using physio-only path at test time
    ...
    out_csv = f"physio_student_classifier_{target_name}_alpha_{alpha}.csv"
