...
def group_kfold_split(X, y, groups, n_splits = 5, label_name = "Arousal"):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups = groups), 1):
        ...
        # Evaluating the model's performance using MSE and Pearson correlation
    ...
    results_df.to_csv(f"{label_name.lower()}_video_regression_groupkfold_results.csv", index=False)
    print(f"\nSaved fold results to {label_name.lower()}_video_regression_groupkfold_results.csv")
