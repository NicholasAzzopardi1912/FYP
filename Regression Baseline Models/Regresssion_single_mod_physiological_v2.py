...
def group_kfold_split(X, y, groups, n_splits = 5, label_name = "Arousal"):
    ...
    results_df.to_csv(f"{label_name.lower()}_physio_regression_groupkfold_results.csv", index=False)
    print(f"\nSaved fold results to {label_name.lower()}_physio_regression_groupkfold_results.csv")
